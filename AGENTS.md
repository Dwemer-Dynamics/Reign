...
- Local commits/pushes to these remotes are normal only when publishing or backup is requested. Initial uploads are held until the complete package has clean-install execution evidence on another computer. Creating/deleting/transferring repositories, visibility changes, force pushes or other remotes require explicit authorization.
