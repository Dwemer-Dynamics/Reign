...
| Shared shipped content | `<Bannerlord>/Modules/ReignBeta/PortraitCache/_shared` |
...
Evidence is under `D:/ReignLaunch/portrait-discovery-fix-20260914/`. `normal-process-before.json` reproduces missing discovery in an independent WMI-launched .NET Framework process. `normal-process-after.json` established the durable record and separate writable portrait root. On 2026-09-15 the user superseded the external shipped-content layout: the record now points `contentRoot` at the module, all 1,228 shared portrait directories live under `Modules/ReignBeta/PortraitCache/_shared`, and writable portraits remain at `D:/Reign/Data/PortraitCache`. Both desktop server shortcuts select `.reign/setup/Start-ReignServer.ps1`.
...
The campaign storage audit must resolve shared art through the installation contract and therefore now expects it beneath the module, while writable campaign portraits remain under `Data/PortraitCache`. The actual portrait catalog and package hash audit must reject any mismatch. The shared WSL migration helper also lacks a native Windows target and showed path/SQL-cast gaps; this run used the retained one-time native helper and full independent database comparison.
