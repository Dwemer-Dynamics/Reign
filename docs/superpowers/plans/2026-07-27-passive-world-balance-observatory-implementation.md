...
- `ReignBetaServer/WorldTestTelemetry.cs` — schema, idempotent counter writes, exact relationship category membership, bounded evidence, and test fixtures.
- `ReignBetaServer/WorldTestRollupWorker.cs` — independent queue/worker, daily rollup finalization, checkpoint caching, reconciliation, and worker status.
- `ReignBetaServer/WorldTestReports.cs` — checkpoint and cross-campaign comparison response builders.

### Existing server files

- `ReignBetaServer/MbtiRelationships.cs` — emit relationship chunk deltas inside existing 500-pair transactions.
- `ReignBetaServer/ContinuousRelationshipWorker.cs` — enqueue completed days and expose simulation-versus-rollup clocks.
- `ReignBetaServer/SocialReputation.cs` — emit rumor funnel counters without changing decisions.
- `ReignBetaServer/ExpandedSocialReputation.cs` — count corrections/counterevidence.
- `ReignBetaServer/RelationshipLifecycle.cs` — classify organic romance/family funnel outcomes.
- `ReignBetaServer/RelationshipDirector.cs` — classify arranged-marriage funnel outcomes.
- `ReignBetaServer/WorldDiplomacyDirector.cs` — classify cadence, intent, proposal, and outcome counters.
- `ReignBetaServer/WorldTest.cs` — replace synchronous full aggregation with materialized reads and extend details.
- `ReignBetaServer/Program.cs` — routes and Control Center presentation.
- `ReignBetaServer/VerificationLab.cs` — source-contract coverage for the new architecture.
- `ReignBetaServer/ReignBetaServer.csproj` — include new sources in copied verification contracts.
